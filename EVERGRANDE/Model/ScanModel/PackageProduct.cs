...
namespace EVERGRANDE
{
    public class PackageProduct
    {
        ...
        public static PackageProduct TryPrase(string itemString, out string errorMsg)
        {
            ...
            else
            {
                ...
                #region 校验DeliveryBarcode

                product.DeliveryBarcode = items[1];
                if (string.IsNullOrEmpty(product.DeliveryBarcode) == true)
                {
                    errorMsg = "纳入标签不能为空。";
                    return null;
                }
        ...
        {
            return string.Format("{1}{0}{2}{0}{3}{0}{4}{0}{5}", StaticInfo.SplitChat, this.Seq, this.DeliveryBarcode, this.ProductionBarcode, this.ScanTime.ToString(StaticInfo.DateTimeFormat), this.ScanAccount);
        }
    }
}
