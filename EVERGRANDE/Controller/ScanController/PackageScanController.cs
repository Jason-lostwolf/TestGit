...
{
    public class PackageScanController : BaseController
    {
        public FrmPackageScanViewModel ViewModel;
        public PackageScanController()
        {
            this.ViewModel = new FrmPackageScanViewModel();
        }
        ...
        private void GetProductList()
        {
            ...
            string fileName = System.IO.Path.Combine(StaticInfo.ProgramPath, StaticInfo.FrmPackageScanDataFileName);
            ...
            List<PackageProduct> list = new List<PackageProduct>();
            if (fileData != null && string.IsNullOrEmpty(fileData.Trim()) == false)
            {
                ...
                if (tempList != null)
                {
                    foreach (string item in tempList)
                    {
                        ...
                        PackageProduct record = PackageProduct.TryPrase(item, out errorMsg);
            ...
            this.ViewModel.ProductList = new System.ComponentModel.BindingList<PackageProduct>(list);
        }
        ...
        public void SaveBarcode()
        {
            ...
            //{
            //Trim标签和ToUpper
            this.ViewModel.Barcode = this.ViewModel.Barcode.Trim().ToUpper();
            ...
            else
            {
                ////前面可能要进行截取操作
                //string msg = string.Empty;
                //string pn = base.ValidatePN(this.ViewModel.Barcode, "纳入标签", out msg);
                //string sn = base.ValidateSN(this.ViewModel.SN, "生产标签条码", out msg);
                ...
                if(sn.Contains(pn)==true)
                {
                    ...
                    PackageProduct p = new PackageProduct();
                    p.DeliveryBarcode = this.ViewModel.Barcode;
        ...
        {
            string msg = string.Empty;
            string result = base.ValidatePN(this.ViewModel.Barcode, "纳入标签", out msg);
            if (string.IsNullOrEmpty(msg) == false)
            {
                Utility.ShowError(msg);
                this.OnUIRefresh(ScanData.FirstBarcode);
        ...
        private void SaveFile(bool isExport)
        {
            ...
            if (isExport == true)
            {
                ...
                fileName = System.IO.Path.Combine(StaticInfo.ExportPath, StaticInfo.PackageExportFilePrefix +  DateTime.Now.ToString(StaticInfo.ExportFileFormat) + StaticInfo.ExportFileExtension);
            }
            else
            {
                ...
                fileName = System.IO.Path.Combine(StaticInfo.ProgramPath, StaticInfo.FrmPackageScanDataFileName);
            }
            StringBuilder sb = new StringBuilder();
            if (isExport == true)
            {
                sb.AppendLine(StaticInfo.FrmPackageScanExportTitle);
            }
            foreach (PackageProduct product in this.ViewModel.ProductList)
            {
